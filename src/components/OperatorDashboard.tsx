...
import { Eye, CheckCircle, AlertTriangle, Package, Download, Search, Filter, X, Calendar, RotateCcw, FileSpreadsheet } from 'lucide-react';
...
export const OperatorDashboard: React.FC = () => {
  ...
  const [acknowledgeDefectId, setAcknowledgeDefectId] = useState<string | null>(null);
  const [scrapAcknowledgeDefectId, setScrapAcknowledgeDefectId] = useState<string | null>(null);
  const [selectedLine, setSelectedLine] = useState<number>(3);
  const [inspectedDefects, setInspectedDefects] = useState<Set<string>>(new Set());
  const [showInspectWarning, setShowInspectWarning] = useState<string | null>(null);
  const [showFilters, setShowFilters] = useState(false);
  const [showReportConfirmation, setShowReportConfirmation] = useState(false);
  const [showStatusDropdown, setShowStatusDropdown] = useState(false);
  const statusDropdownRef = useRef<HTMLDivElement>(null);
  const [selectedStationType, setSelectedStationType] = useState<'Roll' | 'Bundle' | null>(null);
  ...
  const productionLines = allProductionLines.filter(line => line.id === selectedLine);
  ...
  const applyFilters = (defectsList: any[]) => {
    return defectsList.filter(defect => {
      // Product Code filter
      if (filters.productCode && !defect.productCode.toLowerCase().includes(filters.productCode.toLowerCase())) {
        return false;
      }

      // Roll/Bundle Number filter
      if (filters.rollNumber) {
        const numberToCheck = defect.station === 'Roll' ? defect.rollNumber : defect.bundleNumber;
        if (!numberToCheck || !numberToCheck.toLowerCase().includes(filters.rollNumber.toLowerCase())) {
          return false;
        }
      }

      // Job Order Number filter
      if (filters.jobOrderNumber && !defect.jobOrderNumber.toLowerCase().includes(filters.jobOrderNumber.toLowerCase())) {
        return false;
      }
      ...
      if (filters.rollWidth) {
        ...
        if (filters.rollWidth.includes('-')) {
          ...
          const [min, max] = filters.rollWidth.split('-').map(v => parseInt(v.trim()));
      ...
      // Status filter
      if (filters.status.length > 0 && !filters.status.includes(defect.status)) {
        return false;
      }
  ...
  const getDefectsForLine = (lineId: number, station: 'Roll' | 'Bundle') => {
    const lineDefects = defects.filter(defect => 
      defect.productionLine === lineId && 
      defect.station === station
    );
  ...
  const getRemainingDefects = (lineId: number, station: 'Roll' | 'Bundle') => {
    return getDefectsForLine(lineId, station).filter(defect => 
      defect.status.includes('Defect') && (defect.state === 'pending' || defect.state === 'rejected')
    ).length;
  };

  const getRemainingScrap = (lineId: number, station: 'Roll' | 'Bundle') => {
    return getDefectsForLine(lineId, station).filter(defect => 
      defect.status === 'Scrap' && (defect.state === 'pending' || defect.state === 'rejected')
    ).length;
  };
  ...
  const handleScrapAcknowledge = (defectId: string, finalStatus: string, imageFile?: File) => {
    if (user && !isReadOnly) {
  ...
  const generateExcelReport = () => {
    ...
    const allFilteredDefects = [...filteredRollDefects, ...filteredBundleDefects];
    ...
    csvContent += 'วันที่,Production Line,Station,Product Code,Roll/Bundle Number,Job Order Number,Roll Width,Status,State,Acknowledged By,Reviewed By,Comments\n';
    
    allFilteredDefects.forEach(defect => {
      const numberField = defect.station === 'Roll' ? defect.rollNumber : defect.bundleNumber;
      csvContent += `${defect.timestamp},Line ${defect.productionLine},${defect.station},${defect.productCode},${numberField},${defect.jobOrderNumber},${defect.rollWidth},${defect.status},${defect.state},${defect.acknowledgedBy || ''},${defect.reviewedBy || ''},${defect.comments || ''}\n`;
    });
    ...
    const rollNormal = filteredRollDefects.filter(d => d.status === 'Normal').length;
    const bundleNormal = filteredBundleDefects.filter(d => d.status === 'Normal').length;
    
    const rollDefects = filteredRollDefects.filter(d => d.status.includes('Defect')).length;
    const bundleDefects = filteredBundleDefects.filter(d => d.status.includes('Defect')).length;
    
    const rollScrap = filteredRollDefects.filter(d => d.status === 'Scrap').length;
    const bundleScrap = filteredBundleDefects.filter(d => d.status === 'Scrap').length;
    
    csvContent += `จำนวนทั้งหมด,${rollCount},${bundleCount},${totalCount}\n`;
    csvContent += `Normal,${rollNormal},${bundleNormal},${rollNormal + bundleNormal}\n`;
    csvContent += `Defect,${rollDefects},${bundleDefects},${rollDefects + bundleDefects}\n`;
    csvContent += `Scrap,${rollScrap},${bundleScrap},${rollScrap + bundleScrap}\n`;
    ...
    link.setAttribute('download', `production-line-${selectedLine}-report-${dateStr}${filterSuffix}.csv`);
  ...
  const generateStationSpecificReport = (stationType: 'Roll' | 'Bundle') => {
    ...
    csvContent += 'Job Order Number,Roll Width,Status,State,Acknowledged By,Reviewed By,Comments\n';
    
    stationDefects.forEach(defect => {
      const numberField = stationType === 'Roll' ? defect.rollNumber : defect.bundleNumber;
      csvContent += `${defect.timestamp},Line ${defect.productionLine},${defect.station},${defect.productCode},${numberField},${defect.jobOrderNumber},${defect.rollWidth},${defect.status},${defect.state},${defect.acknowledgedBy || ''},${defect.reviewedBy || ''},${defect.comments || ''}\n`;
    });
    ...
    const normalCount = stationDefects.filter(d => d.status === 'Normal').length;
    const defectCount = stationDefects.filter(d => d.status.includes('Defect')).length;
    const scrapCount = stationDefects.filter(d => d.status === 'Scrap').length;
    const qcPassedCount = stationDefects.filter(d => d.status === 'QC Passed').length;
    const pendingCount = stationDefects.filter(d => d.state === 'pending').length;
    const acknowledgedCount = stationDefects.filter(d => d.state === 'acknowledged').length;
    const approvedCount = stationDefects.filter(d => d.state === 'approved').length;
    const rejectedCount = stationDefects.filter(d => d.state === 'rejected').length;
    
    csvContent += `จำนวนทั้งหมด,${totalCount},100%\n`;
    csvContent += `Normal,${normalCount},${totalCount > 0 ? Math.round((normalCount / totalCount) * 100) : 0}%\n`;
    csvContent += `Defect,${defectCount},${totalCount > 0 ? Math.round((defectCount / totalCount) * 100) : 0}%\n`;
    csvContent += `Scrap,${scrapCount},${totalCount > 0 ? Math.round((scrapCount / totalCount) * 100) : 0}%\n`;
    csvContent += `QC Passed,${qcPassedCount},${totalCount > 0 ? Math.round((qcPassedCount / totalCount) * 100) : 0}%\n`;
    csvContent += `\nสถานะการดำเนินการ:\n`;
    csvContent += `รอดำเนินการ,${pendingCount},${totalCount > 0 ? Math.round((pendingCount / totalCount) * 100) : 0}%\n`;
    csvContent += `รับทราบแล้ว,${acknowledgedCount},${totalCount > 0 ? Math.round((acknowledgedCount / totalCount) * 100) : 0}%\n`;
    csvContent += `อนุมัติแล้ว,${approvedCount},${totalCount > 0 ? Math.round((approvedCount / totalCount) * 100) : 0}%\n`;
    csvContent += `ปฏิเสธแล้ว,${rejectedCount},${totalCount > 0 ? Math.round((rejectedCount / totalCount) * 100) : 0}%\n`;
    ...
    if (defectCount > 0) {
      ...
      Object.entries(defectTypes).forEach(([type, count]) => {
        csvContent += `${type},${count},${Math.round((count / defectCount) * 100)}%\n`;
      });
    }
    ...
    const hasFilters = getActiveFilterCount() > 0;
    const filterSuffix = hasFilters ? '-filtered' : '';
    link.setAttribute('download', `${stationType.toLowerCase()}-station-line-${selectedLine}-report-${dateStr}${filterSuffix}.csv`);
  ...
  const getStatusColor = (status: string) => {
    if (status.includes('Defect') || status.includes('Scrap (No Label)')) return 'bg-orange-100 text-orange-800 border-orange-200';
    if (status === 'Scrap') return 'bg-red-100 text-red-800 border-red-200';
    if (status === 'QC Passed') return 'bg-blue-100 text-blue-800 border-blue-200';
    return 'bg-green-100 text-green-800 border-green-200';
  };

  const getStatusIcon = (status: string) => {
    if (status.includes('Defect') || status.includes('Scrap (No Label)')) return <AlertTriangle className="w-4 h-4" />;
  ...
  const currentAcknowledgeDefect = defects.find(d => d.id === acknowledgeDefectId);
  const currentScrapAcknowledgeDefect = defects.find(d => d.id === scrapAcknowledgeDefectId);
  ...
  return (
    <Layout title="Operator Dashboard">
      <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-1">
            <label htmlFor="productionLine" className="text-sm font-medium text-gray-700">
              Production Line:
            </label>
            <select
              id="productionLine"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => handleFilterChange('jobOrderNumber', e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleFilterChange('rollWidth', e.target.value)}
                  placeholder="เช่น 100 หรือ 100-200"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                />
                <p className="text-xs text-gray-500 mt-1">ใส่ค่าเดียว หรือช่วง (100-200)</p>
              </div>
              ...
              <div>
                ...
                <div className="relative" ref={statusDropdownRef}>
                  ...
                  {showStatusDropdown && (
                    <div className="absolute z-10 mt-2 w-full max-h-40 overflow-y-auto border border-gray-200 rounded-lg bg-white shadow-lg">
        ...
        {(user?.role === 'superadmin' || isReadOnly) && (
          <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-1">สร้างรายงานตามสถานี</h3>
                <p className="text-sm text-gray-600">ดาวน์โหลดรายงานแยกตามประเภทสถานีการผลิต</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <button
                  onClick={() => handleStationReportGeneration('Roll')}
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium flex items-center justify-center gap-2 transition-colors whitespace-nowrap"
                >
                ...
                <button
                  ...
                  className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium flex items-center justify-center gap-2 transition-colors whitespace-nowrap"
                >
                  <Download className="w-4 h-4" />
                  สร้างรายงาน Bundle
                </button>
        ...
        {productionLines.map((line) => (
          <div key={line.id} className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
            {/* Production Line Header */}
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-bold">Production Line {line.id}</h2>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Roll</h3>
                  <div className="grid grid-cols-5 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3 text-center border">
                      <div className="text-xl font-bold text-gray-900">{line.rollMetrics.total}</div>
                      <div className="text-xs text-gray-600 font-medium">จำนวน Roll ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-orange-50 rounded-lg p-3 text-center border border-orange-200">
                      <div className="text-xl font-bold text-orange-800">{line.rollMetrics.totalDefect}</div>
                      <div className="text-xs text-orange-700 font-medium">จำนวน Defect ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-red-50 rounded-lg p-3 text-center border border-red-200">
                      <div className="text-xl font-bold text-red-800">{line.rollMetrics.totalScrap}</div>
                      <div className="text-xs text-red-700 font-medium">จำนวน Scrap ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-orange-100 rounded-lg p-3 text-center border border-orange-300">
                      <div className="text-xl font-bold text-orange-700">{getRemainingDefects(line.id, 'Roll')}</div>
                      <div className="text-xs text-orange-600 font-medium">จำนวน Defect ที่รอการตรวจสอบ</div>
                    </div>
                    <div className="bg-red-100 rounded-lg p-3 text-center border border-red-300">
                      <div className="text-xl font-bold text-red-700">{getRemainingScrap(line.id, 'Roll')}</div>
                      <div className="text-xs text-red-600 font-medium">จำนวน Scrap ที่รอการตรวจสอบ</div>
                    </div>
                  </div>
                </div>
                      ...
                      <table className="min-w-full">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Job Order Number</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Roll Width</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {getVisibleDefects(line.id, 'Roll').map((defect) => (
                            <tr key={defect.id} className="hover:bg-gray-50">
                              <td className="px-3 py-2 text-sm font-medium text-gray-900">{defect.productCode}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.rollNumber}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.jobOrderNumber}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.rollWidth}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.timestamp}</td>
                              <td className="px-3 py-2">
                                <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(defect.status)}`}>
                                ...
                                {defect.state === 'rejected' && (
                                  <div className="mt-1">
                                    <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">
                                      {defect.rejectionReason === 'ยังคงเห็น Defect อยู่' ? 'ยังคงเห็น Defect อยู่' : defect.rejectionReason}
                                    </span>
                                  </div>
                                )}
                              </td>
                              <td className="px-3 py-2">
                                <div className="flex space-x-2">
                                  ...
                                  {defect.status !== 'Normal' && !isReadOnly && (defect.state === 'pending' || defect.state === 'rejected') && (
                                    <button
                                      onClick={() => handleAcknowledgeClick(defect.id)}
                                      className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                                        inspectedDefects.has(defect.id)
                                          ? 'bg-orange-500 hover:bg-orange-600 text-white'
                                          : 'bg-gray-400 text-gray-200 cursor-not-allowed'
                                      }`}
                                      disabled={!inspectedDefects.has(defect.id)}
                                    >
                                      {defect.status === 'Scrap (ไม่พบฉลาก)' ? 'จำแนกประเภท Scrap' : 'ยืนยันการแก้ไข'}
                                    </button>
                                  )}
                                  {showInspectWarning === defect.id && (
                                    <div className="absolute z-10 bg-red-100 border border-red-400 text-red-700 px-2 py-1 rounded text-xs mt-8 -ml-16 whitespace-nowrap">
                                      กรุณาตรวจสอบก่อน!
                                    </div>
                                  )}
                                </div>
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Bundle</h3>
                  <div className="grid grid-cols-5 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3 text-center border">
                      <div className="text-xl font-bold text-gray-900">{line.coilMetrics.total}</div>
                      <div className="text-xs text-gray-600 font-medium">จำนวน Bundle ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-orange-50 rounded-lg p-3 text-center border border-orange-200">
                      <div className="text-xl font-bold text-orange-800">{line.coilMetrics.totalDefect}</div>
                      <div className="text-xs text-orange-700 font-medium">จำนวน Defect ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-red-50 rounded-lg p-3 text-center border border-red-200">
                      <div className="text-xl font-bold text-red-800">{line.coilMetrics.totalScrap}</div>
                      <div className="text-xs text-red-700 font-medium">จำนวน Scrap ทั้งหมดในกะนี้</div>
                    </div>
                    <div className="bg-orange-100 rounded-lg p-3 text-center border border-orange-300">
                      <div className="text-xl font-bold text-orange-700">{getRemainingDefects(line.id, 'Bundle')}</div>
                      <div className="text-xs text-orange-600 font-medium">จำนวน Defect ที่รอการตรวจสอบ</div>
                    </div>
                    <div className="bg-red-100 rounded-lg p-3 text-center border border-red-300">
                      <div className="text-xl font-bold text-red-700">{getRemainingScrap(line.id, 'Bundle')}</div>
                      <div className="text-xs text-red-600 font-medium">จำนวน Scrap ที่รอการตรวจสอบ</div>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full">
                        <thead className="bg-gray-100">
                          <tr>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Product Code</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bundle Number</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Job Order Number</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Roll Width</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Timestamp</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {getVisibleDefects(line.id, 'Bundle').map((defect) => (
                            <tr key={defect.id} className="hover:bg-gray-50">
                              <td className="px-3 py-2 text-sm font-medium text-gray-900">{defect.productCode}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.bundleNumber}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.jobOrderNumber}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.rollWidth}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{defect.timestamp}</td>
                              <td className="px-3 py-2">
      ...
      <ConfirmationDialog
        ...
        message={`ยืนยันการดาวน์โหลดรายงานสำหรับ ${selectedStationType} Station - Line ${selectedLine}${getActiveFilterCount() > 0 ? ' (มีการกรองข้อมูล)' : ''}`}
